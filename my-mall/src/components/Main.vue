    ...
    <aside :class="collapsed?'menu-collapsed':'menu-expanded'">
      ...
      <el-menu :default-active="$route.path" class="el-menu-vertical-demo" @open="handleopen" @close="handleclose" @select="handleselect"
        ...
        <template v-for="(item,index) in $router.options.routes" v-if="!item.hidden">
          ...
          <el-menu-item v-if="item.leaf&&item.children.length>0" :index="item.children[0].path" :key='item.leaf'><i :class="item.iconCls"></i>{{item.children[0].name}}</el-menu-item>
        </template>
      </el-menu>
      <!--导航菜单-折叠后-->
      <ul class="el-menu el-menu-vertical-demo collapsed" v-show="collapsed" ref="menuCollapsed">
        <li v-for="(item,index) in $router.options.routes" v-if="!item.hidden" class="el-submenu item" :key='item+index'>
...
.container .main .content-container .breadcrumb-container .content-wrapper {
